...
public class AuthCheckAspect {
    ...
    private AuthCheck getAuthCheckAnnotation(ProceedingJoinPoint joinPoint) {
        ...
        // 取类级别的注解
        annotation = (AuthCheck) joinPoint.getTarget().getClass().getAnnotation(AuthCheck.class);
        if (annotation != null) {
            return annotation;
        }

        // 兜底：类上没注解放方法上（理论上走到这里说明 @within 匹配了类）
        return (AuthCheck) joinPoint.getTarget().getClass().getAnnotation(AuthCheck.class);
    }
}
