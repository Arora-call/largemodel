...
public class ProjectService {
    ...
    /** 为项目创建真实目录和文件，返回文件树 */
    public List<Map<String, Object>> createProjectFiles(Long projectId, List<Map<String, String>> files) {
        ...
        try {
            ...
            log.info("项目 {} 创建完成，共 {} 个文件，目录: {}", projectId, files.size(), projectDir);
        } catch (IOException e) {
    ...
    public List<Map<String, Object>> getFileTree(Long projectId) {
        ...
        try (Stream<Path> stream = Files.walk(projectDir)) {
            stream.filter(Files::isRegularFile).forEach(f -> {
                ...
                file.put("language", detectLangByExt(relativePath));
    ...
    private String detectLangByExt(String path) {
